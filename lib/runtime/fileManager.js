var _ = require('lodash')
    ...
    , q = require('q')
;
function modRequire(lib) {
    try {
        if(moddir) {
            return require(Path.join(moddir, 'node_modules', lib));
        } else {
            return require(lib);
        }
    } catch(e) {
        throw new Error('npm install ' + lib + ' to run this fixture', e);
    }
}
providers.dropbox = {
    client: null
    , init: function(creds) {
        if(!providers.dropbox.client) {
            var dropbox = modRequire('dropbox');
